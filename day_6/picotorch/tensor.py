...
class Tensor:
    ...
    def numpy(self):
        if self.device == 'cuda':
            ...
            return ops_cuda.to_numpy(self._data).reshape(self.shape)
    ...
    def relu(self):
        ...
        def _backward():
            if self.requires_grad:
                if self.device == 'cpu':
                    # CPU: use numpy boolean mask
                    m = (self._data > 0).astype(np.float32)
                    g = dispatch.mul(m, out.grad._data, self.device)
                else:
                    # CUDA: use the dedicated relu_backward kernel
                    from picotorch import ops_cuda
                    g = ops_cuda.relu_backward(self._data, out.grad._data)
                if self.grad is None:
                    self.grad = Tensor(None, device=self.device,
                                       _shape=self.shape, _data_raw=dispatch.zeros(self.size, self.device))
                dispatch.add_inplace(self.grad._data, g, self.device)
        ...
        def _backward():
            if self.requires_grad:
                if self.device == 'cpu':
                    ...
                    # CUDA: copy the single scalar back to host
                    from picotorch import ops_cuda
                    host_val = ops_cuda.to_numpy(out.grad._data)
                    scalar_grad = float(host_val[0])
    ...
    def item(self):
        ...
        if self.device == 'cuda':
            from picotorch import ops_cuda
            return float(ops_cuda.to_numpy(self._data)[0])
