...
def _get_backend(device):
    ...
    if device == 'cpu':
        ...
        if not CUDA_AVAILABLE:
            raise RuntimeError(
                "CUDA backend not available. Build the CUDA library:\n"
                "  cd picotorch/csrc && nvcc -shared -Xcompiler -fPIC "
                "-O2 -o libops_cuda.so ops_cuda.cu"
            )
        return ops_cuda
    else:
...
def to_device(data, src_device, dst_device):
    ...
    CPU→CUDA: numpy array → CudaArray (device memory)
    CUDA→CPU: CudaArray → numpy array
    """
